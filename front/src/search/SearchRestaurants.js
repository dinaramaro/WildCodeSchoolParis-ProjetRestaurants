...
class SearchRestaurants extends Component {
        ...
        axios
          ...
          .then(result => {
            if (this.state.restaurants !== 0) {
              this.allRestaurantsAndFavorites(
                this.state.favoritesUser,
  ...
  allRestaurantsAndFavorites = (favorites, restaurants) => {
    console.log("favorites", favorites);
    console.log("restaurantsTab", restaurants);

    const restaurantsWithFavorites = restaurants.map(restaurant => {
      if (
        favorites.find(favorite => restaurant.id === favorite.restaurant_id)
      ) {
  ...
  onSubmit = e => {
    ...
    axios.get("http://localhost:3001/api/users/1").then(favorites => {
      this.setState({
        favoritesUser: favorites.data
      });
    });
    ...
    axios
      .get(
        `http://localhost:3001/api/restaurants/?result=all${areaUrl}${firstCategoryUrl}${secondCategoryUrl}${ratingUrl}`
      )
      .then(data => {
        this.setState({
          restaurants: data.data,
        ...
        <Form onSubmit={this.onSubmit}>
          <Row className="searchbar">
            ...
            <Col lg="3" md="1">
              <Input
                type="select"
                onChange={this.handleChangeSecondaryCategory}
              >
                <option value="undefined">Sélectionnez une catégorie</option>
